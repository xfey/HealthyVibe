...
日期：2026-05-19  
阶段：MVP 实现完成
状态：MVP Phase 0-7 已完成，进入发布前人工验收
...
- 支持完成后 30 分钟冷却、通知内稍后提醒和 60 分钟活跃无 hook 兜底提醒。
- 每天生成固定任务池，每次只下发一张任务卡。
- 任务完成后展示完成状态或空态，不立即补发新任务。
- 提供今日延寿进度、历史日历、基础设置、关于页和小队排名。
...
- 如果没有处于完成冷却或稍后提醒状态，则下发一个任务。
- 如果上一次通知没有被确认完成，后续 hook 仍继续提醒。
- 用户完成任务后进入 30 分钟冷却，冷却期间不提醒。
- 用户选择 `30分钟后提醒` 或 `两小时后提醒` 后，在稍后提醒到期前不重复提醒，并安排延迟系统通知。
...
- `UserPromptSubmit` 可能通过 stdin 传入 prompt payload；bridge 必须直接丢弃 stdin，不解析、不保存、不上传、不输出。
...
- 触发条件满足后发系统通知。
- 完成冷却或稍后提醒期间收到的 hook event 只记录，不重复发系统通知。
- 通知提供 `已完成`、`30分钟后提醒`、`两小时后提醒` 三个系统 action。
...
状态：MVP 已接入生产 relay。生产地址为 `https://healthyvibe.owlib.ai`。
...
HealthyVibe 第一版不做账号系统，后端只作为轻量 relay / 当日排行榜服务。当前生产 relay 为 Node.js + SQLite，经 nginx 反向代理暴露 HTTPS。
...
- hook bridge 可能接收 hook payload，但只丢弃 stdin；本地 event inbox 只包含 `source`、`event`、`receivedAt`。
- 远端只存匿名小队当天榜单快照。
